...
**Source** - Фізичний елемент управління, що використовується для входу. Крім фізичних елементів управління, ви також можете вказати MAX (завжди повертає 100), MIN (завжди повертає -100), циклічні елементи, триммери, значення каналів, глобальні змінні та інше. Переміщення фізичного елемента управління після вибору джерела автоматично прив’яже його до цього входу. Натисніть **\[Enter]** довго, щоб перейти на початок доступної категорії.

**Weight** - Відсоткове значення ходу стіка, яке потрібно використовувати (часто називається "rates"). Ви також можете натиснути довго **\[Enter]** для показу меню зміни на інший вхід, вісь, потенціометр, MAX (завжди повертає 100), MIN (завжди повертає -100), циклічний вихід, триммер, значення каналу, глобальну змінну та інше.

**Offset** - Значення, яке додається до або віднімається від джерела входу. Ви також можете натиснути довго **\[Enter]** для показу меню зміни на інший вхід, вісь, потенціометр, MAX (завжди повертає 100), MIN (завжди повертає -100), циклічний вихід, триммер, значення каналу, глобальну змінну та інше.

**Curve** - Вказує тип кривої, яка буде використана. Існують такі варіанти кривих:

* **Diff** - Множить тільки діапазон вище або нижче середнього (0) на вказаний %. Натисніть довго **\[Enter]** для вибору значення джерела з іншого входу, осі, глобальної змінної та іншого.
* **Expo** -Значення входу змінюється експоненційно. Збільшення % призведе до плавного нахилу біля середнього (0). Зменшення % призведе до різкого нахилу біля середнього (0). При 0% нахил буде лінійним. Натисніть довго **\[Enter]** для вибору значення джерела з іншого входу, осі, глобальної змінної та іншого.
*   **Func** -
