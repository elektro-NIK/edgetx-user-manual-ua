...
**Звук запуску (Startup sound) -** Якщо ввімкнено (налаштування за замовчуванням), під час запуску радіо відтворюється звук "Welcome to EdgeTX". Відповідний файл знаходиться у `/SOUNDS/??/SYSTEM/hello.wav` (де `??` позначає дволітерний каталог для вашого звукового пакета).
...
* **Тренер вимк. (Trainer Shutdown)** - перевіряє, чи є сигнал тренера активним, під час спроби вимкнення. Подає звуковий та візуальний сигнал, якщо його виявлено.
...
**Заставка (Splash Screen)** - тривалість відображення заставки. Варіанти: **---** (вимкнено), **2s, 3s, 4s, 6s, 8s, 10s, 15s**

**Затримка увімкн. (Power On delay)** - затримка між натисканням кнопки живлення і моментом увімкнення апаратури. Варіанти: **0s, 0.5s, 1s, 2s, 3s**

**Затримка вимкн. (Pwr Off delay)** - затримка між натисканням кнопки живлення і моментом вимкнення апаратури. Варіанти: **0s, 0.5s, 1s, 2s, 3s, 4s**. _Рекомендується встановити щонайменше затримку 1s, щоб запобігти відключенню радіоапаратури при випадковому натисканні кнопки._

**Автовимк. (Power Auto Off)** - якщо цю функцію ввімкнено (за замовчуванням не ввімкнено), і якщо апаратура залишається неактивною, без активного тренера чи телеметрії, вона автоматично вимкнеться після закінчення налаштованого періоду. Ви можете вибрати значення від **0** (вимкнено) до **255** (хвилин).

**Вібро ON/OFF (Power ON/OFF Haptic)** - якщо цю функцію ввімкнено (за замовчуванням), після завершення затримки вмикання та вимикання живлення апаратура вібрує, що вказує на момент вмикання або вимикання.

**ID користув. (Owner ID)** - спеціальний реєстраційний ідентифікатор, що використовується лише для користувачів з модулями ISRM або на передавачах, що підтримують зовнішні модулі FrSky ACCESS. Якщо не налаштовано вручну, буде використано випадкове значення.
...
**ОбКодРеж** (Режим обертального енкодера) (деякі апаратури) - встановлює напрямок обертального енкодера для Ролера. Варіанти:
