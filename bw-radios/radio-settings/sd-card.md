...
* **Призначити зображення** (**Assign bitmap**) - призначає вибраний файл зображення як зображення моделі для поточної вибраної моделі (для 212x128 "широкоекранних" апаратур).
...
* **Прошити \[ціль]** (**Flash \[target]**) — перемикає вибраний файл прошивки у вибраний цільовий модуль. Приклади: Прошити апаратуру, Прошити внутр. Multi.
...
_ПРИМІТКА: Ця папка використовується лише для радіоапаратур з екраном 212x64._
...
Тут записуються файли зображень знімків екрана, які налаштовані у [Спеціальній функції](../model-select/special-functions.md) **Скриншот** (**Screenshot**). За замовчуванням ця папка порожня (за винятком файлу readme.txt).
...
* **Mixes** - у цій папці розміщуються сценарії Lua мікшера. За замовчуванням ця папка порожня (за винятком файлу readme.txt). Додаткову інформацію про сценарії Lua мікшера можна знайти тут: [Custom Mixer Scripts](https://luadoc.edgetx.org/part_i_-_script_type_overview/mix)
